...
def update(req: UpdateRequest):
    ...
    m.save(MODEL_PATH)
    return {"status": "ok"}
# --- RESET ENDPOINT (injected) ---
@app.post("/reset")
def reset():
    """
    Reset server-side learning state:
    - reinitialize the TF model
    - clear any frequency/streak memory
    - remove saved weights file
    """
    global model

    # Try to reuse your existing builder if it exists
    try:
        model = build_model()  # your project helper, if defined
    except Exception:
        # minimal fallback so endpoint still works
        import tensorflow as tf
    ...
    # best-effort clears for common aux state names (ignore if not defined)
    try:
        global freq_counts
        freq_counts = {"Rock": 0, "Paper": 0, "Scissors": 0}
    except Exception:
        pass

    try:
        global streak_state
        streak_state = {"move": None, "len": 0}
    except Exception:
        pass
